...
atras.addEventListener("click", ()=>{
    ...
    if (pagina === 1) atras.disabled = false;
    if (pagina > 1) {
        pagina = pagina - 1;
        const busqueda = buscar.value;
        const url = `https://api.giphy.com/v1/stickers/search?api_key=aMrr0RDuTmYWBJIcohqJrtlRpqndJ7b0&q=${busqueda}&limit=40&page=${pagina}&offset=1&lang=es`;
        leerDatos(url);
    }
});
//btn siguiente
siguiente.addEventListener("click", ()=>{
    ...
    if (pagina === numPag - 1) siguiente.disabled = true;
    if (pagina < numPag) {
        console.log(pagina);
        pagina = pagina + 1;
        const busqueda = buscar.value;
...
//# sourceMappingURL=index.9b217000.js.map
